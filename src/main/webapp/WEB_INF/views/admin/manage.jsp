		...
		<c:forEach var="request" items="${store.content}">
			<div class="col-md-6">
				<div class="card m-2">
					...
					<div class="badge bg-danger text-white position-absolute" style="top: 0.5rem; right: 0.5rem">${request.status}</div>
					</c:when>
					<c:when test="${request.status eq 'approve'}">
					<div class="badge bg-success text-white position-absolute" style="top: 0.5rem; right: 0.5rem">${request.status}</div>
					</c:when>
					
					<c:otherwise>
					<div class="badge bg-info text-white position-absolute" style="top: 0.5rem; right: 0.5rem">${request.status}</div>
					</c:otherwise>
					</c:choose>
