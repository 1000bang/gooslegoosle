...
public class ServiceCenterController {
	...
	public String fetchServiceList(Model model,
			@PageableDefault(size = 6, sort = "id", direction = Direction.DESC) Pageable pageable, @AuthenticationPrincipal PrincipalDetail detail) {
		
		Page<ServiceCenterEntity> service = serviceCenter.getServiceMyList(detail.getUser().getId(), pageable);
...
//	
	@GetMapping("/admin/seviceCenter")
	public String fetchServiceAllList(Model model,
		...
		model.addAttribute("endPage", end);
		model.addAttribute("services", service);
		return "service_center/admin_service_center";
	}	
