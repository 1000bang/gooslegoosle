...
public class CustomAuthFailureHandler extends SimpleUrlAuthenticationFailureHandler {
	...
	public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response,
		...
		if (exception instanceof BadCredentialsException) {
			...
			errorMessage = "내부적으로 발생한 시스템 문제로 인해 요청을 처리할 수 없습니다. 관리자에게 유선 문의하세요.";
		} else if (exception instanceof UsernameNotFoundException) {
			errorMessage = "계정이 존재하지 않습니다. 회원가입 진행 후 로그인 해주세요.";
		} else if (exception instanceof AuthenticationCredentialsNotFoundException) {
			errorMessage = "인증 요청이 거부되었습니다. 관리자에게 유선  문의하세요.";
		} else if (exception instanceof DisabledException) {
			errorMessage = "경고 누적으로 계정이 정지되었습니다. 관리자에게 유선 문의하세요.";
		} else {
			errorMessage = "알 수 없는 이유로 로그인에 실패하였습니다 관리자에게 유선 문의하세요.";
		}
